...
// function prototypes
void input_handler(char[], Record[], int * psize);
void display_menu();
...
int main(){
  ...
  int size = 0;
	...
	while(strcmp(user_input, "99") != 0){
		...
		input_handler(user_input, records, &size);
	}
  free(records);
	return 0;
}
...
void input_handler(char input[], Record * records, int * psize){

	// TODO: Modify this function as you need

	if(!strcmp(input, "1"))
		add_new_item(records, psize);
  else if(!strcmp(input, "2"))
    add_new_item_file(records, psize);
  else if(!strcmp(input, "3"))
	  print_inventory_status(records, psize);
  else if(!strcmp(input, "4"))
    import_inventory_status(records, psize);
    ...
    records = rescale_system(records);
  else if(!strcmp(input, "12"))
    records = reset_system(records);
...
void display_menu(){
  ...
  printf(" 2. Add new item from a data file\n");
  ...
  printf(" 9. Sort items\n");
  printf(" 10. Calculate total inventory asset\n");
  printf(" 11. Scale up system capacity\n");
  printf(" 12. System Reset\n");
